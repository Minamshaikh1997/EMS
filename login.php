...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (ems_login_is_limited() || ems_db_login_is_limited($conn, $email)) {
    ...
    // ADMIN LOGIN
    // ==========================
    $adminStmt = $conn->prepare("SELECT * FROM admin WHERE email=? LIMIT 1");
    $adminStmt->bind_param('s', $email);
    $adminStmt->execute();
    $adminQuery = $adminStmt->get_result();
    ...
    if ($adminQuery && mysqli_num_rows($adminQuery) > 0) {
        ...
        if (password_verify($password, $admin['password'])) {
            ...
            $_SESSION['admin_role_original'] = isset($admin['role']) && !empty($admin['role']) ? $admin['role'] : 'Admin';
            $_SESSION['admin_role'] = ems_canonical_admin_role($_SESSION['admin_role_original']);
    ...
    // EMPLOYEE LOGIN
    // ==========================
    $empStmt = $conn->prepare("SELECT * FROM employees WHERE email=? LIMIT 1");
    $empStmt->bind_param('s', $email);
    $empStmt->execute();
    $empQuery = $empStmt->get_result();
...
.logo-icon.has-company-logo { background: white; padding: 7px; }
...
<div class="login-container">
    <div class="login-card">
        ...
        <div class="logo-section">
            <div class="logo-icon" id="companyLogoBox">
                <i class="fas fa-briefcase"></i>
            </div>
            <h2 id="companyName">Welcome Back</h2>
        ...
        <div class="footer-text">
            &copy; 2026 <span id="footerCompanyName">Employee Leave System</span>. All rights reserved.
        </div>
...
<script src="assets/company_branding.js" defer></script>
</body>
</html>
