...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['adj_id'])) {
    ...
    if (mysqli_num_rows($adj) === 0) {
        ...
        if (!isset($error)) {
            ...
            ems_audit($conn, 'attendance_adjustment.admin_' . strtolower(str_replace(' ', '_', $action_desc)), 'attendance_adjustment', $adj_id, [
                'status' => $new_status,
...
<title>Admin Adjustments - EMS</title>
...
.badge-cancelled { background: rgba(100,116,139,.12); color: var(--secondary); }
...
<div class="main-content" id="mainContent">
    <header class="header">
        <div class="header-left">
            ...
            <h4>Admin Adjustments <span>/ All Attendance Adjustments</span></h4>
        </div>
    ...
    <div class="page-content">
        <div class="card-modern">
            <div class="card-header-custom">
                <h5><i class="fa fa-shield-alt"></i> Admin - All Attendance Adjustments</h5>
                <small class="text-muted">Full control: Approve, Reject, Hold, or Cancel requests</small>
            </div>
            <div class="card-body-custom">
                ...
                <div class="table-responsive">
                    <table class="table table-modern" id="adjustmentsTable">
                        <thead>
                            <tr>
                                <th>Request #</th>
                                <th>Employee</th>
                        ...
                        <tbody>
                            <?php if ($adjustments && mysqli_num_rows($adjustments) > 0): ?>
                                <?php while ($row = mysqli_fetch_assoc($adjustments)): ?>
                                    ...
                                    $supervisor_status = $row['supervisor_comment'] ? 'Reviewed' : 'Pending';
                                    $supClass = 'badge-pending';
                                    if ($row['status'] === 'Approved') $supClass = 'badge-approved';
                                    elseif ($row['status'] === 'Rejected') $supClass = 'badge-rejected';
                                    elseif ($row['status'] === 'Hold') $supClass = 'badge-hold';
                                    ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if (!empty($row['attachment'])): ?>
                                                <a href="../uploads/adjustments/<?php echo $row['attachment']; ?>" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill"><i class="fas fa-paperclip"></i></a>
                                            <?php else: ?>
                                                <span class="text-muted small">—</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <span class="badge-modern <?php echo $supClass; ?>"><?php echo $supervisor_status; ?></span>
...
<form method="POST" class="d-inline">
                                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(ems_csrf_token()) ?>">
                                                    ...
                                                    <div class="d-flex gap-1 flex-wrap">
                                                        <button type="submit" name="action" value="approve" class="btn btn-success btn-sm rounded-pill px-3" onclick="return confirm('Approve? Attendance will be updated.')"><i class="fas fa-check"></i></button>
                                                        <button type="submit" name="action" value="reject" class="btn btn-danger btn-sm rounded-pill px-3" onclick="return confirm('Reject?')"><i class="fas fa-times"></i></button>
                                                        <button type="submit" name="action" value="hold" class="btn btn-warning btn-sm rounded-pill px-3" onclick="return confirm('Hold?')"><i class="fas fa-pause"></i></button>
                                                        <button type="submit" name="action" value="cancel" class="btn btn-outline-secondary btn-sm rounded-pill px-3" onclick="return confirm('Cancel this request?')"><i class="fas fa-ban"></i></button>
                                                    </div>
                                                </form>
                                            <?php else: ?>
                                ...
                                <tr>
                                    <td colspan="11">
                                        <div class="empty-state">
                                            ...
                                            <p class="text-muted">No attendance adjustment requests have been submitted yet.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
